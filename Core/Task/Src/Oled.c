...
void StartOledTask(void *argument)
{
    ...
    {
        OLED_NewFrame();
        snprintf(message, sizeof(message), "Real:%.2fC", realTemp);
        OLED_PrintString(1, 1, message, &font13x13, OLED_COLOR_NORMAL);
        snprintf(message, sizeof(message), "Knob:%dC", show_target);
        OLED_PrintString(1, 20, message, &font13x13, OLED_COLOR_NORMAL);
        snprintf(message, sizeof(message), "Set:%.1fC", targetTemp);
